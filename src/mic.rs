...
impl Microphone {
    ...
    pub fn consume(&mut self) {
        ...
        let needed_samples = should_have_processed - self.num_samples_processed;
        ...
        if needed_samples > available {
            ...
            let note = (((note % 12) + 12) % 12) as usize;
            let notes = ["A", "Bb", "B", "C", "C#", "D", "Eb", "E", "F", "F#", "G", "Ab"];
            let note = notes[note];
            println!("note: {}, freq: {}", note, freq);

            self.num_samples_processed += needed_samples;
